...
//static uint8_t _USART1_RX_BUF[30u/18u * BSP_USART1_DMA_RX_BUF_LEN];
...
//			{
//				RC_DataProcess(_USART1_DMA_RX_BUF[0]);
//			}
//            
////			if(this_time_rx_len == RC_FRAME_LENGTH)
////			{
...
//            if(this_time_rx_len == RC_FRAME_LENGTH)
//			{
//				RC_DataProcess(_USART1_DMA_RX_BUF[1]);
//            }
//            //			if(this_time_rx_len == RC_FRAME_LENGTH)
////			{
...
//{
//    FIFO_Init(&_USART1_RX_FIFO, (void *)_USART1_RX_BUF, RC_FRAME_LENGTH, 30u/18u); 
//}
...
void USART1_Configuration(uint32_t baud_rate)
{
    ...
    USART_DeInit(USART1);
		usart.USART_BaudRate = baud_rate;   //SBUS 100K baudrate
		usart.USART_WordLength = USART_WordLength_8b;
		usart.USART_StopBits = USART_StopBits_1;
		usart.USART_Parity = USART_Parity_Even;
		usart.USART_Mode = USART_Mode_Rx;
    ...
    dma.DMA_Channel= DMA_Channel_4;
    dma.DMA_PeripheralBaseAddr = (uint32_t)&(USART1->DR);
    dma.DMA_Memory0BaseAddr = (uint32_t)&USART1_RX_BUF[0];
    dma.DMA_DIR = DMA_DIR_PeripheralToMemory;
...
//    isopened.usartRC=1;
    ...
    {        
        ...
        RC_DataProcess(USART1_RX_BUF);        
    }
}
unsigned char USART1_RX_BUF[BUF_Size];
